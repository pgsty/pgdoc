...
<xsl:template match="indexterm" mode="index-primary">
  ...
  <fo:block xmlns:rx="http://www.renderx.com/XSL/Extensions" xmlns:axf="http://www.antennahouse.com/names/XSL/Extensions">
    ...
    <xsl:choose>
      <xsl:when test="starts-with(primary, 'pg_plan_advice.')">
        <xsl:call-template name="pgdoc.break.index.name">
...
<!-- Permit line breaks in long PG20 plan-advice index names without changing
     authored identifiers, index keys, or copy-visible characters. -->
<xsl:template name="pgdoc.break.index.name">
  <xsl:param name="text"/>
  <xsl:choose>
