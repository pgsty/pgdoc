#!/usr/bin/env python3
"""Apply reviewed Chinese translations after regenerating the pinned PG12 input.
...
def main():
    ...
    for name, record in mapping.items():
        ...
        if digest(data) != record['input_sha256']:
            raise ValueError(f'{name}: regenerated PG12 input hash mismatch')
        text = data.decode()
    ...
    for path, text in outputs:
        ...
        print(f'Localized generated PG12 table: {path.name} {digest(text.encode())}')
