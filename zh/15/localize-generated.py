#!/usr/bin/env python3
"""Apply reviewed Chinese translations after regenerating the pinned PG15 input.
...
def main():
    ...
    for name, record in mapping.items():
        ...
        if digest(data) != record['input_sha256']:
            raise ValueError(f'{name}: regenerated PG15 input hash mismatch')
        text = data.decode()
    ...
    for path, text in outputs:
        ...
        print(f'Localized generated PG15 table: {path.name} {digest(text.encode())}')
